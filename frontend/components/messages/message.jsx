...
const Message = ({ messages, message, i, users, currentUserId, editing, setEditing, setDeleting, reacting, setReacting, postReaction, deleteReaction, reply, formHeight }) => {
  let dateStamp;
  ...
  if (users[message.senderId]) {
    return <>
      {i === 0 || message.seconds - Object.values(messages)[i - 1].seconds >= 60 || message.senderId !== Object.values(messages)[i - 1].senderId || message.repliedMessageId ?
      <div key={message.id}>
        {i === 0 || message.date !== Object.values(messages)[i - 1].date ?
          ...
          : null}
        <li key={`message-${message.id}`} className={"message" + (editing === message.id ? " editing" : "") + (reacting.messageId === message.id ? " reacting" : "")}>
          ...
          {message.repliedMessageId ? <div className="replied-message">
            ...
            <p>{messages[message.repliedMessageId].body}</p>
          </div> : null}
      ...
      : <li key={`message-${message.id}`} className={"message followup-message" + (editing === message.id ? " editing" : "") + (reacting.messageId === message.id ? " reacting" : "")}>
        {editing === message.id ?
